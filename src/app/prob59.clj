...
(defn ioc
  "index of coincidence as n(n-1)/k(k-1) n(j) is the frequency of letter j in the text and k is the length of the text"
  [freqs text-len]
  (let [denom (* text-len (dec text-len))]
    (reduce + (map #(/ (* % (dec %)) denom) freqs))))

(defn ioc-for-keylen
  "calculates average ioc for all L 'columns' of text for the guessed key length L"
  [data key-len]
...
(defn guess-ley-len [ciph]
  (let [iocs (->> (range 1 1000)
    ...
    (->> (map-indexed vector iocs)
         (filter #(> (second %) 0.06)) ;;0.063 is the theoretical "unnormalized" IOC for English
         (first)
         (first)
         (inc))))
...
(defn eval-col-freqs
  "statistical distance 'sum(f*n)' between the theoretical letter frequencies lfq and actual freqs in a single column col"
  [lfq col]
  (->> (frequencies col)
